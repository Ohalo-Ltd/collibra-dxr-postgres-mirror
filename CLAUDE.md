...
- `org.postgresql.Driver` **is** on Collibra's classpath, but egress is **HTTP(S)-only**: raw TCP to any host/port (tested 5432/443/8080 against portquiz.net and Cloud SQL) hangs → "The connection attempt failed / Read timed out". Only `HttpURLConnection` to https works.
